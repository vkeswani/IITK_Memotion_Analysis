...
listing =listing0 + listing1 + listing2
...
number= size(listing)
list1=[[],[]] * (number//2+1) #困难之处！！！！！！！！创建一个以二维数组为元素的一维数组来存储矩阵
k=0
#图像数据预处理
...
immatrix = array([array(list1[i]).flatten() for i in range(number)],'f')
...
for i in range(len(df)):
    imgl = [[0 for row in range(img_rows)] for col in range(img_cols)]
    im = Image.open(path + '/' + df['image_name'][i])   
    img = im.resize((img_rows,img_cols))#将每张图片调整为300*300尺寸
    img.save(path + '/' + df['image_name'][i])
...
immatrix2 = array([array(list2[i]).flatten() for i in range(len(df))],'f')
cnn_input= pd.DataFrame(immatrix2)
immatrix2 = immatrix2.reshape(immatrix2.shape[0], img_rows, img_cols, 1)
immatrix2 = immatrix2.astype('float32')
immatrix2 /= 15
...
immatrix3 = array([array(list2[i]).flatten() for i in range(len(df))],'f')
immatrix3 = immatrix3.reshape(immatrix3.shape[0], img_rows, img_cols, 1)
immatrix3 = immatrix3.astype('float32')
immatrix3 /= 15
...
ytrue= np.argmax(Y_test,axis=1)-1

all_y_img= model.predict(immatrix2)
tst_y_img= model.predict(immatrix3)
...
#%%
all_img= pd.DataFrame(all_y_img,columns=['img-1','img0','img1'])
tst_img= pd.DataFrame(tst_y_img,columns=['img-1','img0','img1'])

all_img.to_csv('all_distribution_image.csv',index= None)
tst_img.to_csv('test_distribution_image.csv',index= None)

#%%
cnn_input.to_csv('cnn_input.csv',index= None)
